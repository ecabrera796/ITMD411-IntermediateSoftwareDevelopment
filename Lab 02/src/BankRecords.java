...
public class BankRecords extends Client{
    ...
    private static BankRecords mInstance;
    //format time stamps
    ...
    public static BankRecords getInstance(){
        if(mInstance == null){
            synchronized (BankRecords.class){
                mInstance = new BankRecords();
            }
        }
        return mInstance;
    }
    ...
    public void printData() {
        ...
        for (int i = 0; i<robjs.length; i++){
            if(robjs[i] == null) break; // hit last element in list
            if(i<25)
                System.out.println(heading);
            System.out.println(robjs[i]);
        }
    }
    ...
    public void printData(int recordsToPrint){
        ...
        String heading = String.format("%-8s\t%-4s\t%-7s\t%-11s\t%-12s\t%-8s\t%-9s\t%-4s\t%-9s" +
        							   ...
        							   "MORTGAGE", "PEP");
        System.out.println(heading);
        for (int i = 0; i<robjs.length && i<recordsToPrint; i++){
            if(robjs[i] == null) break; // hit last element in list
            System.out.println(robjs[i]);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        BankRecords.getInstance().readData();
        boolean cont = true;
        ...
        do {
            ...
            try{
                ...
                if(records < 1){
                    ...
                    BankRecords.getInstance().printData(records);
                }
            }catch (Exception ex){
                continue;
            }
        ...
        System.out.println(String.format("\nTotal records in bank-detail.csv are %d", BankRecords.array.size())+"\n");
        //time stamp and name
